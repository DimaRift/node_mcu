...
void setup() {
  ...
  WiFi.mode(WIFI_STA);
  WiFiMulti.addAP("Livebox-0c4c", "3795E5608375FFA132E01A2356");     // Connexion au réseau wifi
...
void loop() {
  ...
  if (isnan(h) || isnan(t)) {         // test si des valeurs ont été récupérées
    ...
    if((WiFiMulti.run() == WL_CONNECTED)) {   // Si le wifi est connecté
          ...
          http.begin("http://51.75.126.58/dht11/store_temp.php");   // connexion au serveur
